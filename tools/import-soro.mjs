 ...
 *
 * Everything imported lands as `draft: true` with empty `topics`. That is
 * deliberate: the feed carries neither topics nor links into this site, and
 * nothing here should publish technical claims about sewer work without a
 * person reading it first.
 *
 *   npm run blog:import                  # uses DEFAULT_FEED (or SORO_FEED_URL)
...
const FILE = val("--file");
// This site's feed, from Soro -> Settings -> Other Platform -> Manage. Env/flag still win.
const DEFAULT_FEED = ""; // set to this site's own Soro feed when one exists
const FEED = val("--feed") || process.env.SORO_FEED_URL || DEFAULT_FEED;

const die = (m) => { console.error(`\n  ${m}\n`); process.exit(1); };
if (!FEED) die("No Soro feed set for this site yet. Pass --feed <url> or set SORO_FEED_URL.");
...
let SITE_HOST = "asap-trenchless.com";
try {
...
async function main() {
  ...
  for (const item of items) {
    ...
    const frontmatter = [
      ...
      date ? `date: ${date}` : null,
      "topics: []            # REQUIRED — pick from existing topics before publishing",
  ...
  if (written.length) {
    console.log("  All are draft: true with empty topics — they will NOT build until");
    console.log("  someone sets topics and clears the draft flag.\n");
  }
}
